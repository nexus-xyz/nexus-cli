...
impl RequestTimer {
    ...
    pub fn can_proceed(&mut self) -> bool {
            ...
            (self.config.max_requests, self.config.time_window)
        {
            ...
            self.request_times.retain(|&time| now.duration_since(time) <= time_window);
...
mod tests {
    ...
    #[test]
    fn test_rate_limiting_without_server_delay() {
        let config = RequestTimerConfig::_requests_per_window(2, Duration::from_millis(100));
        let mut timer = RequestTimer::new(config);
