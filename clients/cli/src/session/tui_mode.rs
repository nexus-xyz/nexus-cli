...
use crate::version::checker::{check_for_new_version, check_version_constraints};
use crossterm::{
...
pub async fn run_tui_mode(
    ...
    let (version_update_available, latest_version) =
        if let Some(_version_result) = check_version_constraints(current_version).await {
            // Version constraints violated - will be handled by main.rs validation
            (false, None)
        } else if let Some(message) = check_for_new_version(current_version).await {
