...
async fn start(
    ...
    if let Err(e) = analytics::measure_flops_and_set_user_properties(&env, client_id.clone(), num_workers).await {
        eprintln!("Warning: Failed to set user properties for analytics: {}", e);
