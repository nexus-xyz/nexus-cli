...
use crate::version::validate_version_requirements;
...
async fn start(
    ...
    let config =
        Config::resolve(node_id, &config_path, &orchestrator_client).await?;

    // 3. Session setup (authenticated worker only)
    let session = setup_session(config, env, max_threads).await?;
