...
impl VersionRequirements {
    ...
    pub async fn fetch() -> Result<Self, VersionRequirementsError> {
        ...
        let config: VersionRequirements = response.json().await
            .map_err(|e| VersionRequirementsError::FetchError(e.to_string()))?;
        Ok(config)
    }

    /// Check all version constraints and return the most severe violation
    pub fn check_version_constraints(&self, current_version: &str, latest_version: Option<&str>, release_url: Option<&str>) -> Result<Option<VersionCheckResult>, VersionRequirementsError> {
        ...
        for constraint in &self.version_constraints {
            ...
            if current < min_version {
                ...
                let message = self.format_message(&constraint.message, current_version, &constraint.min_version, latest_version, release_url);
    ...
    fn format_message(&self, template: &str, current_version: &str, min_version: &str, latest_version: Option<&str>, release_url: Option<&str>) -> String {
        template
            .replace("{current}", current_version)
            .replace("{min_version}", min_version)
            .replace("{latest}", latest_version.unwrap_or("unknown"))
            .replace("{release_url}", release_url.unwrap_or("https://github.com/nexus-xyz/nexus-cli/releases"))
    }
...
mod tests {
    ...
    fn test_version_comparison() {
        ...
        let result = config.check_version_constraints("0.9.1", None, None).unwrap();
        assert!(result.is_none()); // No violations
        
        let result = config.check_version_constraints("0.8.9", None, None).unwrap();
        assert!(result.is_some());
        assert!(matches!(result.unwrap().constraint_type, ConstraintType::Warning));
        
        let result = config.check_version_constraints("0.7.9", None, None).unwrap();
        assert!(result.is_some());
    ...
    fn test_version_parsing() {
        ...
        let result = config.check_version_constraints("v1.0.0", None, None).unwrap();
        assert!(result.is_none()); // No violations
    }
    ...
    fn test_constraint_priority() {
        ...
        let result = config.check_version_constraints("0.6.0", None, None).unwrap();
        assert!(result.is_some());
        assert!(matches!(result.unwrap().constraint_type, ConstraintType::Blocking));
    }
        ...
        let config = VersionRequirements {
            version_constraints: vec![
                VersionConstraint {
                    min_version: "1.0.0".to_string(),
                    constraint_type: ConstraintType::Notice,
                    message: "Version {current} < {min_version}. Latest: {latest}. URL: {release_url}".to_string(),
                    start_date: None,
                },
            ],
        };

        let result = config.check_version_constraints("0.9.0", Some("1.1.0"), Some("https://example.com")).unwrap();
        assert!(result.is_some());
