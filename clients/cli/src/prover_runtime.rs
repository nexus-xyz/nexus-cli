...
impl TaskFetchState {
    ...
    fn should_log_queue_status(&mut self, tasks_in_queue: usize) -> bool {
        tasks_in_queue < LOW_WATER_MARK && self.last_queue_log_time.elapsed() >= self.queue_log_interval
    }
    ...
    fn increase_backoff_for_rate_limit(&mut self) {
        self.backoff_duration = std::cmp::min(self.backoff_duration * 2, Duration::from_millis(60000));
    }

    fn increase_backoff_for_error(&mut self) {
        self.backoff_duration = std::cmp::min(self.backoff_duration * 2, Duration::from_millis(30000));
    }
}
...
async fn log_queue_status(
    ...
    let message = if time_since_last >= state.backoff_duration {
        ...
        format!("Queue low: {} tasks, last fetch {}s ago (retry every {}s)", 
            tasks_in_queue, 
            time_since_secs,
            backoff_secs
        )
...
) -> Result<(), bool> { // bool indicates if caller should return
    if tasks.is_empty() {
        let _ = event_sender
            .send(Event::task_fetcher_with_level(
...
async fn handle_fetch_error(
    ...
    if matches!(error, OrchestratorError::Http { status: 429, .. }) {
        ...
        let _ = event_sender
            .send(Event::task_fetcher_with_level(
                format!("Rate limited (429), backing off for {} seconds", state.backoff_duration.as_secs()),
                EventType::Error,
                LogLevel::Warn
            ))
            .await;
        ...
        let event = Event::task_fetcher_with_level(
            format!("Failed to fetch tasks: {}, retrying in {} seconds", error, state.backoff_duration.as_secs()),
            EventType::Error,
            log_level
        );
        if event.should_display() {
