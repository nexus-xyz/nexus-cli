...
async fn process_fetched_tasks(
    ...
    for task in tasks {
        ...
        tokio::spawn(track_got_task(task.clone(), environment.clone(), client_id.to_string()));
...
async fn process_proof_submission(
        ...
        .await
    {
        Ok(_) => {
            ...
            tokio::spawn(track_proof_submission_success(task.clone(), environment.clone(), client_id.to_string()));
...
async fn handle_submission_success(
    ...
    tokio::spawn(track_proof_accepted(task.clone(), environment.clone(), client_id.to_string()));
...
async fn handle_submission_error(
    ...
    tokio::spawn(track_proof_submission_error(task.clone(), msg.clone(), status_code, environment.clone(), client_id.to_string()));
