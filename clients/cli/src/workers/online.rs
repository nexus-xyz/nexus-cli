...
/// Check if task was already submitted successfully
...
async fn submit_proof_to_orchestrator(
        ...
        .await
    {
        Ok(_) => {
            ...
            handle_submission_success(task, event_sender, successful_tasks, environment, client_id)
                .await;
        }
        Err(e) => {
...
async fn process_proof_submission(
    ...
    if check_duplicate_submission(&task, successful_tasks, event_sender).await {
