...
use super::requirements::{VersionCheckResult, VersionRequirements};
...
/// Check version constraints and determine what to display to user
pub async fn check_version_constraints(current_version: &str) -> Option<VersionCheckResult> {
    // Try to fetch requirements and check constraints
    match VersionRequirements::fetch().await {
        Ok(requirements) => {
            // Create a basic version checker to get latest version info
            let version_checker = VersionChecker::new(current_version.to_string());

            match version_checker.check_latest_version().await {
                Ok(release) => requirements
                    ...
                    .flatten(),
                Err(_) => {
                    // If we can't fetch latest version, just check constraints without it
                    requirements
        ...
        Err(_) => {
            // If we can't fetch requirements, don't show any version messages
            None
        }
    }
}
